...
func main() {
	...
	createDockerRegistrySecretsByK8sClient(clientset, "./registry-secret.yaml")
}
...
func createSecretsFile() error {
	...
	fmt.Printf("Waiting for generate ssl files for docker-registry...\n")
	err = cmd.Wait()
	return err

}

func createDockerRegistrySecretsByK8sClient(cli *kubernetes.Clientset, filename string) {
	_, err := os.Stat(filename)
	if err != nil {
		if os.IsNotExist(err) {
			fmt.Printf("Error creating docker-registry secrets: file %s not exist.\n", filename)
			panic(err.Error())
		} else {
			fmt.Printf("Error creating docker-registry  secrets: %v.\n", err)
			panic(err.Error())
		}
	}
	...
	if err == nil {
		...
		if errors.IsNotFound(err) {
			...
			panic(fmt.Errorf("Error creating docker-registry  secrets: can not detect secret %s in namespace %s exist or not", secret.Name, secret.Namespace).Error())
		}
	}
}
